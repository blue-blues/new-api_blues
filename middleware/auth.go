...
func authHelper(c *gin.Context, minRole int) {
	...
	if username == nil {
		...
		if user != nil && user.Username != "" {
			...
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "No permission to perform this operation, invalid access token",
			})
			c.Abort()
			return
		}
	...
	if role.(int) < minRole {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "No permission to perform this operation, insufficient privileges",
		})
		c.Abort()
		return
	}
	...
	return func(c *gin.Context) {
		...
		if len(parts) > 1 {
			if model.IsAdmin(token.UserId) {
				...
				abortWithMessage(c, http.StatusForbidden, "Regular users do not support specifying channels")
				return
			}
		}
