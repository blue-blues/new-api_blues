...
func Register(c *gin.Context) {
	...
	if !config.PasswordRegisterEnabled {
		c.JSON(http.StatusOK, gin.H{
			"message": "Administrator has disabled password registration, please use third-party account verification for registration",
			"success": false,
		})
		return
	}
...
func GetUser(c *gin.Context) {
	...
	if myRole <= user.Role && myRole != model.RoleRootUser {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "No permission to access information of users with same or higher privilege level",
		})
		return
	}
	c.JSON(http.StatusOK, gin.H{
...
func UpdateUser(c *gin.Context) {
	...
	if myRole <= originUser.Role && myRole != model.RoleRootUser {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "No permission to update user information of same or higher privilege level",
		})
		return
	}
	if myRole <= updatedUser.Role && myRole != model.RoleRootUser {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "No permission to elevate other users' privilege level to equal or higher than your own",
		})
		return
	}
...
func CreateUser(c *gin.Context) {
	...
	if user.Role >= myRole {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "Cannot create users with privilege level greater than or equal to your own",
		})
		return
	}
...
func ManageUser(c *gin.Context) {
	...
	case "promote":
		if myRole != model.RoleRootUser {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "Regular admin users cannot promote other users to administrator",
			})
			return
		}
...
func AdminTopUp(c *gin.Context) {
	...
	if req.Remark == "" {
		req.Remark = fmt.Sprintf("Top up via API %s", common.LogQuota(int64(req.Quota)))
	}
