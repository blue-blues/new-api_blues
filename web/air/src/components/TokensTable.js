...
const TokensTable = () => {
  ...
  const link_menu = [
    ...
    { node: 'item', key: 'ama', name: 'AMA Ask Heaven', value: 'ama' },
    {
    ...
    },
    {
      ...
      render: (text, record, index) => (
        <div>
          ...
          >Copy</Button>
          <SplitButtonGroup style={{ marginRight: 1 }} aria-label="Item Operation Button Group">
            ...
            <Dropdown trigger="click" position="bottomRight" menu={
              [
                ...
                {
                  node: 'item', key: 'ama', name: 'AMA Ask Heaven (BotGem)', onClick: () => {
  ...
  const onCopy = async (type, key) => {
    ...
    // } else {
    //     showWarning('Failed to copy to clipboard, please copy manually. Token has been filled in search box.');
  ...
  const onOpenLink = async (type, key) => {
    ...
    switch (type) {
      ...
      default:
        if (!chatLink) {
          showError('Administrator has not set chat link');
          return;
        }
        url = defaultUrl;
    }
    ...
    <>
      ...
      <Form layout="horizontal" style={{ marginTop: 10 }} labelPosition={'left'}>
        ...
        <Button label="Query" type="primary" htmlType="submit" className="btn-margin-right"
                onClick={searchTokens} style={{ marginRight: 8 }}>Query</Button>
      </Form>
      ...
      <Table style={{ marginTop: 20 }} columns={columns} dataSource={pageData} pagination={{
        ...
        formatPageText: (page) => `${page.currentStart} - ${page.currentEnd} of ${tokens.length} items`,
